...
import { Trophy, Flame, Clock, Award, Play, RotateCcw, CheckCircle } from 'lucide-react';
...
export const WorkoutSummaryModal: React.FC<WorkoutSummaryModalProps> = ({
  ...
  const getAccuracyFeedback = (score: number) => {
    if (score >= 90) return { title: t.setCompletedTitle, desc: t.setCompletedDescMaster };
    if (score >= 75) return { title: t.setCompletedTitle, desc: t.setCompletedDescGood };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      ...
      >
        ...
        <p className={`text-xs sm:text-sm max-w-md mb-6 leading-relaxed ${isDark ? 'text-neutral-300' : 'text-neutral-600'}`}>
          {feedback.desc}
        </p>
        ...
        <div className="w-full grid grid-cols-3 gap-3 mb-6">
          ...
          >
            ...
            <span className={`text-xs mt-0.5 ${isDark ? 'text-neutral-400' : 'text-neutral-500'}`}>
              {t.accuracyScore}
            </span>
          </div>
        ...
        <div className="w-full flex flex-col sm:flex-row gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-neutral-950 font-bold text-sm sm:text-base transition-all shadow-lg shadow-emerald-500/25"
          >
          ...
          <button
            ...
            onClick={onRepeat}
            className={`flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl font-semibold text-sm sm:text-base border transition-all ${
              isDark
